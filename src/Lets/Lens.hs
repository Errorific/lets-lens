...
fmapT ::
  ...
  -> t a
  -> t b
fmapT f = getIdentity . traverse (pure . f)
...
over ::
  ...
  -> s
  -> t
over fi f = getIdentity . fi (Identity . f)
...
foldMapT ::
  ...
  -> t a
  -> b
-- foldMapT f ta = _ (f <$> ta)
foldMapT f t = getConst (traverse (Const . f) t)
